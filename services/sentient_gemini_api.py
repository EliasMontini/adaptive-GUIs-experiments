...
def adapt_step(user_profile: Dict[str, Any],
    ...
    """
    system = (
        "We are developing an experiment for a hands-on exercise using the Universal Robots UR5e Cobot involving screwdriver installation and a subsequent programming task. You must adapt the content displayed to the participants on the UI (User Interface) by adjusting the visibility of text, images, and videos."
        "The exercise consists of the following steps: Safety Check, Hardware Installation (mounting the screwdriver), System Power-Up, Software Configuration (creating Installation and Program files), and Tool Definition (setting the Payload)."
        "The interface includes the following elements for each step, which can be set to visible (true) or hidden (false):"
        "short_text: A concise, one-line summary of the task. long_text: A detailed, multi-step explanation. image_teach_pendant (also called image_single_pieces): the image of the teach pendant with the highlighting of the buttons to use. "
        "image_environment (also called image_assembly): A static image showing the physical robot, its end-effector, or the task environment. video: A dynamic video demonstration of the operation."
